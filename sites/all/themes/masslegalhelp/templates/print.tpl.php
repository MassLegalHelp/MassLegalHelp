  ...
  <body style="margin:8px;">
    ...
    <?php global $language;
		  if($language->language=='en'){
			$print_logo = '/images/print-logo.gif';
		  }		  
		  if($language->language=='pt'){
			$print_logo = '/images/logo_mlh-color_PORT.gif';
		  }
		  if($language->language=='vi'){
			$print_logo = '/images/logo_mlh-color_VIET.gif';
		  }
		  if($language->language=='es'){
			$print_logo = '/images/logo_mlh-color_SPA.gif';
		  }
		  if($language->language=='ru'){
			$print_logo = '/images/logo_ru-print.gif';
		  }
		  if($language->language=='ht'){
			$print_logo = '/images/logo_haitian-print.gif';
		  }
		  if($language->language=='zh-hant'){
			$print_logo = '/images/logo_chinese-print.gif';
		  }
...
if(arg(0)=='print'){
	?>
      <div class="print-logo"><img src="<?php print base_path(). path_to_theme(). $print_logo ?>" /></div>
	  <?php } ?>
    ...
    <?php endif; ?>
    <div class="print-content"><?php if(arg(0)=='print'){ print $content; } ?></div>
    ...
    <?php endif; ?>
	<?php //print theme('print_url_list'); 
	if($content){
	$a = getLinks($content);
	if($a){ ?>
    <div class="print-links"><br /><h3>Links</h3><br />
	<?php foreach($a as $key=>$detail){
		if($detail){
			echo $detail.' =>      <a href="$key">'.$key.'</a><br />';
		}
	}
	?></div><?php } } ?>
